...
func toolAnnotations(name string) *mcp.ToolAnnotations {
	switch name {
	case "system_info", "computer_info", "computer_screenshot", "task_read", "task_wait", "task_list", "fs_read", "fs_list", "fs_search", "checkpoint_read":
	...
	case "computer_move", "computer_click", "computer_scroll", "computer_type", "computer_key", "task_start", "task_send", "task_stop":
...
func New(caller Caller) *mcp.Server {
	...
	}, &mcp.ServerOptions{Instructions: `Use task_start for commands that may take time, then poll task_read with next_offset. Do not restart a task merely because the MCP request or chat reconnects. Use checkpoint_write at meaningful development milestones so another agent can resume the workspace.`})
